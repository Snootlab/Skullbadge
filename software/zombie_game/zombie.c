  ...
  while(1)								//rgb_timeout is ++ed every 0.5s approx.
	{
		...
		//test purpose
   		...
   		DDRB&=~(_BV(0x04));
		if(PINB&0x04)state.mode_fct==RECEIVE; else state.mode_fct==EMMIT;
		 DDRB|=(_BV(0x04));
		...
		if(refresh_counter>=120) //24 = 1minute. Normal must be 5mins or 10mins (test purpose)
			{
				update_health();
				refresh_counter=0;
			}
...
unsigned char medic_time (unsigned char save_timeout)
{
 ...
 while (state.mode_fct==EMMIT)
 {
   if(timeout_medic<=10)
   {
      ...
      tempo_ten_us(1000);
   }else{RGB_alternative_emt.red=RGB_off.red;RGB_alternative_emt.green=RGB_off.green;RGB_alternative_emt.blue=1;}
